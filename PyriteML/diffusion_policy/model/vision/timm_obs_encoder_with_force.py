    ...
    def __init__(
        ...
        if fuse_mode == "mlp":
            ...
            if self.v_feature_dim == 0 and self.f_feature_dim == 0:
                ...
                n_v_features = len(rgb_keys) * shape_meta["sample"]["obs"]["sparse"][first_rgb_key][
                    "horizon"
                ]
                n_f_features = len(wrench_keys)

                self.linear_projection = nn.Linear(
                    self.v_feature_dim * n_v_features + self.f_feature_dim * n_f_features,
                    max(self.v_feature_dim, self.f_feature_dim), # these two should be the same when both are available
                )
                if position_encoding == "learnable":
                    self.position_embedding = torch.nn.Parameter(
                        torch.randn(n_v_features + n_f_features, max(self.v_feature_dim, self.f_feature_dim))
                    )
